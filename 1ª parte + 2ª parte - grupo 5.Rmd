...
```{r}
subset <- data_norm[1:20, 1:20]

distxy <- dist(subset, method = "euclidean")
hc = hclust(distxy)
plot(hc) 

```
Visualização de um clustering hierarquico com base no subset criado.


# CLUSTERING HIERARQUICO c/TRANSPOSTA
```{r}
transpose <- t(data_norm)
subset <- transpose[1:20, 1:20]
...
wss = c()
for (k in 2:15) wss = c(wss, sum(kmeans(subset, centers=k)$withinss) )
plot(2:15, wss, type="b", xlab="Num Clusters", ylab="WSS") 

clust_res = kmeans(subset, 2)
clust_res$cluster
```
O ponto de "cotovelo" é o ponto no gráfico onde a adição de mais clusters não resulta em uma redução significativa na variância explicada. Neste caso será o 3, sendo que a partir desse ponto, adicionar mais clusters provavelmente não fornecerá um ganho significativo no agrupamento dos dados.


## CLUSTERING K-MEANS c/TRANSPOSTA
```{r}
transpose <- t(data_norm)
subset <- transpose[1:20, 1:20]
...
clust_res$cluster
```
O ponto de "cotovelo" é o ponto no gráfico onde a adição de mais clusters não resulta em uma redução significativa na variância explicada. Neste caso será o 7, sendo que a partir desse ponto, adicionar mais clusters provavelmente não fornecerá um ganho significativo no agrupamento dos dados.
...
pcares <- prcomp(subset, scale = T)
summary(pcares) #importancias da componentes

pcares2 <- princomp(scale(subset))
summary(pcares2) #importancias da componentes
plot(pcares2)
pcares2$loadings #loadings
biplot(pcares2)

svdres= svd(scale(subset))
svdres$v #as colunas de v são equivalentes aos loadings resultantes da PCA

plot(pcares$rotation[,1], svdres$v[,1], pch=19, xlab="PC1", ylab="SV1-dir") #as colunas de v são equivalentes aos loadings resultantes da PCA

min(which(summary(pcares)$importance[3,]>0.9)) #número minimo de componentes para explicar 90% da variabilidade do modelo
...
## ANÁLISE PREDITIVA
```{r}
transpose_subset <- transpose[row.names(transpose) %in% row.names(metadata), ]
subset_genes <- rownames(transpose_subset)
metadata_subset <- metadata[subset_genes, ]
rownames(metadata_subset)
```
Aqui igualamos as linhas do dataset da transposta com o do metadata.

```{r}
transpose_subset_df <- as.data.frame(transpose_subset)
metadata_subset_df <- data.frame(coluna = metadata_subset$vital_status, stringsAsFactors = FALSE)
res_df <- data.frame(transpose_subset_df, metadata_subset_df)

```
De seguida juntamos a coluna viral_status (Alive or Dead) dos metadados ao dataset da transposta.


```{r}
set.seed(12345)
ind = sample(2, nrow(res_df), replace=TRUE, prob=c(0.7, 0.3))
trainData = res_df[ind==1,]
testData = res_df[ind==2,]
table(trainData$coluna)
table(testData$coluna)
```


## Método dos k vizinhos mais próximos
```{r}
knn_pred = knn(trainData[,1:4], testData[,1:4], trainData$coluna)
knn_pred #resultado: valores previstos para a variável de saída


pecc = function(obs,pred)sum(obs==pred)/length(obs)
t = table(knn_pred, testData$coluna)
t
pecc(knn_pred, testData$coluna) #medidas de erro kNN
```


## TREINO DO MODELO
```{r}
trainMatrix <- as.matrix(trainData[, 1:4])
trainLabel <- trainData$coluna

testMatrix <- as.matrix(testData[, 1:4])
testLabel <- testData$coluna
...
```{r}
logitModel <- glmnet(trainMatrix, trainLabel, family = "binomial")
```
...
### Previsão do modelo
```{r}
logitPred <- predict(logitModel, newx = testMatrix, type = "response")
logitPred <- ifelse(logitPred > 0.5, 1, 0)
```


### Avaliação da accuracy do modelo
```{r}
logitAccuracy <- sum(logitPred == testLabel) / length(testLabel)
logitAccuracy
```
Neste caso o valor da accuracy é 0, o que significa que os valores de testLabel são diferentes de logitPred, isto pode querer dizer que o modelo não está a prever corretamente os dados.
...
dtModel <- rpart(trainLabel ~ ., data = trainData[, 1:4])

dtPred <- predict(dtModel, newdata = testData[, 1:4], type = "class")

dtAccuracy <- sum(dtPred == testLabel) / length(testLabel)
dtAccuracy
```
dtAccuracy apresenta uma accuracy de 0.5.

```{r}
rpart.plot(dtModel)
```
No gráfico é observável 74% de Alive contra 26% de Dead.
